...
public class S0611Controller {
	...
	public String getTest5(S0611VO vo, Model model) {
		if (vo.getGender() == 1 || vo.getGender() == 3)
			vo.setStrGender("남자");
		else
			vo.setStrGender("여자");
	 ...
	 * 
	 * @PostMapping("/test8") public ModelAndView postTest8(S0611VO vo,
	 ...
	 * 
	 * mv.addObject("vo", vo); mv.addObject("name", name);
	 * 
	 * return mv; }
	 */
	...
	public String getTest11(Model model,
		...
		System.out.println("회원가입 폼 창으로 이동");
	...
	// @GetMapping("/test14/{mid}____________{pwd}")
